	...
	if ( ! empty( $floor ) && is_array( $floor ) ) {
		foreach ( $floor as $key => $floor ) {
			if ( ! empty( $floor[ $provider->markups->prefix . 'floor_name' ] ) ) {
					...
					<div class="card-header" id="floor<?php echo esc_attr( $key ); ?>">
						...
						<div class="floor_listeo">
							<ul>
								...
								<li><?php echo esc_html__( 'Beds: ', 'resido' ) . esc_html( $floor['wperesds_floor_beds'] ); ?></li>
								<?php } ?>
								<?php if ( ! empty( $floor['wperesds_floor_baths'] ) ) { ?>
								<li><?php echo esc_html__( 'Baths: ', 'resido' ) . esc_html( $floor['wperesds_floor_baths'] ); ?></li>
								<?php } ?>
								<?php if ( ! empty( $floor['wperesds_floor_area'] ) ) { ?>
								<li><span><?php echo esc_html( $floor['wperesds_floor_area'] ) . ' ' . esc_html__( 'sqft', 'resido' ); ?></span></li>
								<?php } ?>
							</ul>
						</div>
					</div>
				...
				echo esc_html__( 'No Floor Plan Included', 'resido' );
			}
		}
	}
	?>
