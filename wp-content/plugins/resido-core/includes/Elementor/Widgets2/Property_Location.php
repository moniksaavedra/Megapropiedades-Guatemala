...
class Property_Location extends Widget_Base {
	...
	public function get_location( $type ) {
		$get_rlistings_location = get_terms( 'listing_location', array( 'hide_empty' => 0 ) );
		$roptions               = array();
			...
			<div class="container">
				...
				<div class="row">
					...
					foreach ( $settings['items'] as $item ) {
						...
						$item_background = ( $item['image']['id'] != '' ) ? wp_get_attachment_image( $item['image']['id'], 'full' ) : $item['image']['url'];

						$city = get_term_by( 'id', $location_id, 'listing_location' );

						if ( $city ) {
							if ( $city->parent ) {
								$parent        = get_term_by( 'id', $city->parent, 'listing_location' );
								$city_location = $city->name . ', ' . $parent->name;
							} else {
								...
								<div class="location-property-wrap">
									...
									<div class="location-property-content">
										<div class="lp-content-flex">
											<h4 class="lp-content-title"><?php echo $city_location; ?></h4>
											<?php
											if ( $city->count <= 1 ) {
